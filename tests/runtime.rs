...
const COMPONENT_ID: ComponentInstanceId =
    ComponentInstanceId(1);

const ROOT_ID: NodeId =
    NodeId(100);

const PADDING_ID: NodeId =
    NodeId(101);

const STACK_ID: NodeId =
    NodeId(102);

const TEXT_ID: NodeId =
    NodeId(103);

const BUTTON_ID: NodeId =
    NodeId(104);

const INCREMENT_ACTION_ID: ActionId =
    ActionId(200);
...
fn builder_constructs_nested_tree() {
    ...
    assert!(
        matches!(
            &padding.kind,
            ViewNodeKind::Padding(_)
        ),
    );

    assert_eq!(
        padding.children.len(),
        1,
    );
...
fn builder_reports_unclosed_nodes() {
    ...
    let error =
        builder
            .finish()
            .expect_err(
                "閉じていないNodeがあるため失敗すること",
            );

    assert_eq!(
        error,
        TreeBuilderError::UnclosedNodes,
    );
}
...
fn builder_reports_multiple_roots() {
    ...
    assert_eq!(
        error,
        TreeBuilderError::MultipleRoots,
    );
}
...
fn runtime_starts_without_tree() {
    #[allow(unused_mut)] let mut runtime =
        ViewRuntime::new(
            COMPONENT_ID,
        );

    assert!(
        runtime.tree().is_none(),
    );
}
...
fn node_ids_remain_stable_after_rebuild() {
    ...
    let first_stack =
        &first_padding.children[0];

    let second_stack =
        &second_padding.children[0];
    ...
    assert_eq!(
        first_stack.children[0].id,
        second_stack.children[0].id,
    );

    assert_eq!(
        first_stack.children[1].id,
        second_stack.children[1].id,
    );
